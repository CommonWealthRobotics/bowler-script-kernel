...
public class MuJoCoBowlerIntegrationTest {
	...
	public void test() throws Exception {
		...
		List<CSG> parts = (List<CSG>) ScriptingEngine.gitScriptRun(CSGDatabase.getInstance(),
				"https://github.com/madhephaestus/VexHighStakes2024.git", "fieldElements.groovy");
		terrain = (ArrayList<CSG>) ScriptingEngine.gitScriptRun(CSGDatabase.getInstance(),
				"https://github.com/madhephaestus/VexHighStakes2024.git", "field.groovy");
		com.neuronrobotics.sdk.common.Log.debug("Parts size = " + parts.size());
		// terrain.add(new Cube(10000,10000,100).toCSG().toZMax());
		free.addAll(parts);
		MuJoCoPhysicsManager manager = new MuJoCoPhysicsManager(CSGDatabase.getInstance(), "javaCadTest", bases, free,
				terrain, new File("./physicsTest"));
