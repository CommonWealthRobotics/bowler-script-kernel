...
public class CaDoodleVitamin {
	...
	public CSG get(String typencoming, String defaultValue, ArrayList<Object> args) {
		...
		StringParameter word = new StringParameter(instance, name + string, strValue, listVitaminSizes);
		...
		if (args.size() > 1) {
			...
			if (!(Boolean) object.get("PreventBomAdd")) {
				...
				// VitaminLocation vl = new VitaminLocation(false, name, type,
				// word.getStrValue(), new TransformNR());
		...
		CSG part;
		try {
			// com.neuronrobotics.sdk.common.Log.debug("Generating Vitamin "+type+"
			// "+word.getStrValue()+" for vitamin named "+name);
				...
				public CSG regenerate(CSG previous) {
					...
					if (t == null) {
						com.neuronrobotics.sdk.common.Log
								.debug(" Error, type is null, previous " + name2 + " has no parameters somehow??");
					}
