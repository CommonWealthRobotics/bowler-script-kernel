...
public class GeometrySimplification {
	 ...
	 *            the incoming CSG
	 * @param MMVoxel
	 *            the mm dimentions of the voxel
	...
	/**
	 * RE-mesh an STL file in place This modifys the original STL
	 *
	 * @param stlout
	 *            the file in which he STL is stored
	 * @param MMVoxel
	 *            the mm dimention of the voxels
	 * @throws Exception
	 */
	...
	public static File simplifySVG(File incoming, double threshhold) {
		try {
			...
			List<String> args = Arrays.asList(inkscape.getAbsolutePath(), "--actions",
					"\"select-all:all;path-simplify:threshold=" + threshhold
							+ ";;export-overwrite;export-do;quit-inkscape\"",
					incoming.getAbsolutePath());
			legacySystemRun(null, inkscape.getAbsoluteFile().getParentFile(), System.out, args);
			args = Arrays.asList(inkscape.getAbsolutePath(), "--export-plain-svg", "--export-type=svg", "--vacuum-defs",
					"--export-filename=" + svg.getAbsolutePath(), incoming.getAbsolutePath());
			legacySystemRun(null, inkscape.getAbsoluteFile().getParentFile(), System.out, args);
			return svg;
		} catch (Exception e) {
