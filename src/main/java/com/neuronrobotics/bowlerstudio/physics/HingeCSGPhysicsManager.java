	...
	public void update(float timeStep) {
		...
		if (constraint != null && getController() != null && !flagBroken) {
			...
			if (constraint.getAppliedImpulse() > getMuscleStrength()) {
				...
				setConstraint(null);
				getCore().add(this);
				com.neuronrobotics.sdk.common.Log.error("ERROR Link Broken, Strength: " + getMuscleStrength()
						+ " applied impluse " + constraint.getAppliedImpulse());
			}
		} else if (constraint != null && flagBroken) {
			constraint.enableAngularMotor(false, 0, 0);
		}
		// com.neuronrobotics.sdk.common.Log.error("Constraint = "+constraint+"
		// controller= "+getController()+" broken= "+flagBroken);
	}
