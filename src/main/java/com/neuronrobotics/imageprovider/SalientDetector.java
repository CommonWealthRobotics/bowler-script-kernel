  ...
  public ArrayList<Rect> SortFindContours(ArrayList<MatOfPoint> a, ArrayList<Rect> b, Mat c,
    ...
    if (a.size() != 0 && a.size() < 100) {
      ...
      for (int i = 0; i < a.size(); i++) {
        ...
        if (i == 0) {
          contourFinal.add(a.get(i));
        } else if (rectHold.tl().y > Horizon) {
          contourFinal.add(a.get(i));
        }
      }
      ...
      for (int i = -1; i < FinalSize; i++) {
        ...
        if (area >= minArea) { // size check
          ...
          if (test.width < newX && test.height < newY) { //test.br().y > Horizon && test
            ...
            if (test.x <= 0 || test.y <= 0 || test.br().x >= c.cols() || test.br().y >= c.rows()) {
              addThis = false;
            }
            if (addThis == true) {
              b.add(test);
          ...
          for (int a1 = 0; a1 < STAGE1_BOXES.size(); a1++) { // process those small
            ...
            for (int cc = 0; cc < 3; cc++) {    // ITERATE THROUGH COLOR RANGES AND STUFF
              ...
              if (cc == 0) {
                ...
                Core.inRange(colorResult, white_min2, white_max2, colorResult);
              } else if (cc == 2) {
                Core.inRange(colorResult, white_min3, white_max3, colorResult);
              }
